...
object Player extends App {
    ...
    while(true) {
        ...
        if (math.abs(vspeed) >= 40 && powerReturn < 4){
            powerReturn += 1
        } else if (math.abs(vspeed) <= 30 && power > 0){
            powerReturn -= 1
        }
        
        println("0 " + powerReturn) // 2 integers: rotate power. rotate is the desired rotation angle (should be 0 for level 1), power is the desired thrust power (0 to 4).
    }
}
